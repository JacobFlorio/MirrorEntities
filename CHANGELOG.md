...
**What changed:** A closer read of the engine's novel contribution — the temporal reasoning engine in `narrative_memory.py` (inspired by an earlier persistent-memory and temporal-reasoning project of mine) — and a scan of the `temporal_alerts` table turned up two seed sources the v0.3 audit missed.
